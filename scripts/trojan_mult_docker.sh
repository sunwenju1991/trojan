...
EOF
	...
	# Docker mode: use mounted certificate, skip acme.sh issue/installcert
	mkdir -p /usr/src/trojan-cert /usr/src/trojan-temp
	if [ ! -s /usr/src/trojan-cert/fullchain.cer ] || [ ! -s /usr/src/trojan-cert/private.key ]; then
	    red "==================================="
	    red "Docker模式：未找到已挂载的证书文件"
	    red "需要存在："
	    red "/usr/src/trojan-cert/fullchain.cer"
	    red "/usr/src/trojan-cert/private.key"
	    red "==================================="
	    exit 1
	fi
	green "Docker模式：检测到已挂载证书，跳过 acme.sh 证书申请"
...
EOF
	...
	# Docker mode: skip systemd service generation
	green "Docker模式：跳过 systemd service 生成与 systemctl 启动"
	green "Trojan 主程序路径：/usr/src/trojan/trojan"
	green "Trojan 配置文件路径：/usr/src/trojan/server.conf"
	green "后续请使用前台命令启动：/usr/src/trojan/trojan -c /usr/src/trojan/server.conf"
	green "======================================================================"
...
if [ "$release" == "centos" ]; then
	...
	ufw_status="" # Docker mode: skip ufw systemctl check
...
true # Docker mode: skip systemctl enable nginx
nginx -s stop >/dev/null 2>&1 || true
green "======================="
blue "请输入绑定到本VPS的域名"
green "======================="
...
if [ $real_addr == $local_addr ] ; then
	...
	true # Docker mode: skip systemctl restart trojan
	nginx >/dev/null 2>&1 || true
    else
    	red "申请证书失败"
    fi
...
function remove_trojan(){
    ...
    red "同时卸载安装的nginx"
    red "================================"
    true # Docker mode: skip systemctl stop trojan
    true # Docker mode: skip systemctl disable trojan
